...
DHCP_BOOL          :=  env_var_or_default('DHCP', "true")
...
update:
    ...
    if [[ {{ MASTERS }} -eq 1 && {{ WORKERS }} -eq 0 ]]; then 
      ...
      yq -i '.rendezvousIP = "{{ BOOTSTRAP_IP }}"' {{OKUB_INSTALL_PATH}}/agent-config.yaml
    fi
...
pxe:
    ...
    if [ -f {{OKUB_INSTALL_PATH}}/agent-config.yaml ]; then
        ...
        if [[ {{ DHCP_BOOL }} == "TRUE" ]]; then NETWORK_CONFIG="ip=eno1:dhcp"; fi
